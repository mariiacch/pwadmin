...
const getUsers= async()=>{
   const res= await axiosInstance.get("ventaSemanal/ventas")
   try{
...
const filtrar=(terminoBusqueda)=>{
    let resultadosBusqueda=tablaUsuarios.filter((elemento)=>{
        if(elemento.createdAt.toString().toLowerCase().includes(terminoBusqueda.toLowerCase())
...
//filtra la data , los users que se queden son los que no coinciden con el user que se selecciono  setUsuarios(usuarios.filter(user=>user._id!==userSeleccionado._id));
  const eliminar = async()=>{
   ...
   try{
   await axiosInstance.delete(Url + userSeleccionado._id )
  ...
  const editar = async()=>{
    ...
    try{
      await axiosInstance.put(Url + userSeleccionado._id,userSeleccionado )
    ...
    return (
       ...
       <div className="ConsultTable">
            ...
            <div className="ConsultHeader">
                <div className="titleConsult">
                    <h2> Consulta de Ventas diarias</h2>
                ...
                <div className="buscadorInput">
                    ...
                    required
                    placeholder="Busqueda por Fecha de Registro "
                    value={busqueda}
          ...
          <thead>
            <tr>
              ...
              <th className="tableHeader bg-grey">Fecha de Registro</th>  
          ...
          {usuarios &&
            usuarios.map((user)=>(
              <tr key={user._id}>
                ...
                <td className="sellTd  " >{user.createdAt}</td>
     ...
     <Modal  className="overlayModal" isOpen={modalEliminar}>
        <ModalBody className="modalBodyEliminar" >
         <span className='bg-grey'> ¿Estás Seguro que deseas eliminar el Registro {userSeleccionado && userSeleccionado.username}? </span>

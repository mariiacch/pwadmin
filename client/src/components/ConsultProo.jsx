...
const getPro= async()=>{
    const res= await axiosInstance.get("proovedor/proovedors")
    try{
...
//filtra la data , los users que se queden son los que no coinciden con el user que se selecciono  setUsuarios(usuarios.filter(user=>user._id!==userSeleccionado._id));
  const eliminar = async()=>{
    ...
    try{
    await axiosInstance.delete(Url + proSeleccionado._id )
  ...
  const editar = async()=>{
    ...
    try{
      await axiosInstance.put(Url + proSeleccionado._id, proSeleccionado)
